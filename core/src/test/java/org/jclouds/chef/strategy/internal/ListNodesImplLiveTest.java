...
public class ListNodesImplLiveTest extends BaseChefLiveTest<ChefApi> {
   ...
   public void testExecuteConcurrentlyWithExecutorService() {
      assertTrue(size(strategy.executeConcurrently(testExecutorService)) > 0, "Expected one or more elements");
   }

   public void testExecuteConcurrentlyWithListeningExecutorService() {
      assertTrue(size(strategy.executeConcurrently(testListeningExecutorService)) > 0, "Expected one or more elements");
   }
}
